...
##### 將上一個專案`/public`底下的`/css`、`/js` 複製過來，不用複製 `/front`
...
- `AdminController`新增`Login`和`home`的方法
    ...
    public function login(Request $req){
        ...
        $manager = (new Manager())->getAdmin($req->userId, $req->pwd);

        if(empty($manager)){
            return back()->withInput()->withErrors(["error"=>"帳號密碼錯誤"]);
            exit;
        }
